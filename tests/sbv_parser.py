...
class SBVParserTestCase(GenericParserTestCase):
    ...
    def test_sbv_invalid_format(self):
        self.assertRaises(
            webvtt.errors.MalformedFileError,
            webvtt.from_sbv,
            self._get_file(u'invalid_format.sbv')
        )

    def test_sbv_total_length(self):
        self.assertEqual(
            webvtt.from_sbv(self._get_file(u'sample.sbv')).total_length,
            16
        )

    def test_sbv_parse_captions(self):
        self.assertEqual(
            len(webvtt.from_srt(self._get_file(u'sample.srt')).captions),
            5
        )
    ...
    def test_sbv_missing_timeframe_line(self):
        self.assertRaises(
            ...
            webvtt.from_sbv,
            self._get_file(u'missing_timeframe.sbv')
        )
    ...
    def test_sbv_missing_caption_text(self):
        self.assertRaises(
            webvtt.errors.MalformedCaptionError,
            webvtt.from_sbv,
            self._get_file(u'missing_caption_text.sbv')
        )
    ...
    def test_sbv_invalid_timestamp(self):
        self.assertRaises(
            webvtt.errors.MalformedCaptionError,
            webvtt.from_sbv,
            self._get_file(u'invalid_timeframe.sbv')
        )

    def test_sbv_timestamps_format(self):
        vtt = webvtt.from_sbv(self._get_file(u'sample.sbv'))
        self.assertEqual(vtt.captions[1].start, u'00:00:11.378')
        self.assertEqual(vtt.captions[1].end, u'00:00:12.305')
    ...
    def test_sbv_get_caption_text(self):
        vtt = webvtt.from_sbv(self._get_file(u'sample.sbv'))
        self.assertEqual(vtt.captions[1].text, u'Caption text #2')

    def test_sbv_get_caption_text_multiline(self):
        vtt = webvtt.from_sbv(self._get_file(u'sample.sbv'))
        self.assertEqual(
            vtt.captions[2].text,
            u'Caption text #3 (line 1)\nCaption text #3 (line 2)'
        )
        self.assertListEqual(
            vtt.captions[2].lines,
            [u'Caption text #3 (line 1)', u'Caption text #3 (line 2)']
        )
